<script>
	...
	import {
		Plus, Lock, Unlock, Trash2, Users,
		ChevronDown, ChevronUp, Check, X,
		Calendar, Map, LogOut, Clock as ClockIcon,
		Image as ImageIcon, Upload, QrCode,
		Sun, Moon, Search
	} from 'lucide-svelte';
	...
	function handleScanSuccess(text) {
		...
		if (p) {
			...
			if (s) {
				const cost = calcPlayerCost(s, getParticipants(s.id), p.needs_racket);
				stopScanner();
	...
	async function handleCreate() {
		if (!newTitle.trim()) { formError = 'Title wajib diisi'; return; }
		...
		db.sessions.flatMap(s => 
			getParticipants(s.id)
				...
				.map(p => ({ 
					...
					sessionDate: s.date,
					cost: calcPlayerCost(s, getParticipants(s.id), p.needs_racket)
				}))
		)
	);
</script>
...
{#if showScanner}
	...
	<div class="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center p-6 animate-fade-in">
		<div class="w-full max-w-sm flex flex-col items-center">
			<div class="mb-8 text-center">
				<h3 class="text-white text-xl font-black mb-2">Scan Member Pass</h3>
				<p class="text-white/50 text-xs">Arahkan kamera ke QR Code di HP peserta</p>
			</div>

			<div id="reader" class="w-full aspect-square bg-white/5 rounded-3xl overflow-hidden border-2 border-white/20 shadow-2xl relative">
				<!-- Guide Overlay -->
...
<div class="max-w-4xl mx-auto px-5">
	...
	<header class="pt-8 pb-5 animate-fade-in">
		<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
			<div>
				<h1 class="text-2xl font-bold text-text-primary leading-tight">Admin Dashboard</h1>
			...
			<div class="flex flex-wrap items-center gap-3">
				...
				<div class="relative flex-1 min-w-[180px]">
					<Search class="absolute left-3 top-1/2 -translate-y-1/2 text-text-tertiary" size={14} />
					<input 
						type="text" 
				...
				>
					{#if db.theme === 'dark'}<Sun size={20} />{:else}<Moon size={20} />{/if}
				</button>

				<form method="POST" action="/admin/login?/logout" use:enhance>
					<button type="submit" class="w-11 h-11 rounded-xl bg-danger/5 flex items-center justify-center text-danger hover:bg-danger/10 transition-all active:scale-95" title="Logout">
						<LogOut size={20} />
					</button>
				</form>
			</div>
	...
	<!-- Stats -->
	{#if db.isReady}
		<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 animate-fade-in-up" style="animation-delay:100ms">
			...
			<div class="bg-surface rounded-2xl p-4 border border-border/50 shadow-sm">
				<p class="text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1 text-navy">Pending Proofs</p>
				<p class="text-lg font-black {pendingVerifications.length > 0 ? 'text-warning' : 'text-text-tertiary'} leading-tight">
		...
		{#if pendingVerifications.length > 0}
			<section class="mb-8 animate-scale-in">
				<div class="flex items-center justify-between mb-3 px-1">
					<h2 class="text-[10px] sm:text-xs font-black text-warning uppercase tracking-[0.2em] flex items-center gap-2">
						<ClockIcon size={14} /> Pending Verifications
					</h2>
					<span class="px-2 py-0.5 rounded-full bg-warning text-white text-[9px] sm:text-[10px] font-bold animate-pulse">
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
					{#each pendingVerifications as p}
						...
						>
							<div class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-warning/20 flex items-center justify-center text-warning font-black text-xs sm:text-sm">
								{p.name.charAt(0).toUpperCase()}
							</div>
							<div class="flex-1 min-w-0">
								<p class="text-[11px] sm:text-xs font-bold text-text-primary truncate">{p.name}</p>
								<p class="text-[8px] sm:text-[9px] text-text-tertiary uppercase tracking-tight truncate">{p.sessionTitle}</p>
							</div>
							<div class="text-right">
								<p class="text-[9px] sm:text-[10px] font-black text-navy">{formatCurrency(p.cost)}</p>
								<span class="text-[7px] sm:text-[8px] font-bold text-warning uppercase">Review Proof →</span>
							</div>
						</button>
					{/each}
				</div>
	...
	<div class="grid grid-cols-2 gap-3 mb-5 animate-fade-in-up">
		...
		>
			{#if showCreateForm}<X size={16} />Cancel{:else}<Plus size={16} />New Session{/if}
		</button>
		<button
		...
		<div class="bg-surface rounded-3xl border border-border/50 shadow-sm p-5 mb-5 animate-scale-in">
			...
			<form onsubmit={(e) => { e.preventDefault(); handleCreate(); }} class="space-y-4">
				<div>
					...
					<input id="session-title" type="text" bind:value={newTitle} placeholder="e.g. Friday Night Smash 🏸"
				...
				<div>
					...
					<input id="session-date" type="date" bind:value={newDate}
				...
				<div class="grid grid-cols-2 gap-3">
					<div>
						<label for="session-time" class="block text-xs font-medium text-text-secondary mb-1.5">Start</label>
						<input id="session-time" type="time" bind:value={newTime}
							class="w-full px-4 py-3 bg-bg rounded-2xl border border-border/50 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-navy/20 transition-all" />
					</div>
					<div>
						<label for="session-end" class="block text-xs font-medium text-text-secondary mb-1.5">End</label>
						<input id="session-end" type="time" bind:value={newEndTime}
							class="w-full px-4 py-3 bg-bg rounded-2xl border border-border/50 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-navy/20 transition-all" />
					</div>
				</div>
				<div>
					<label for="session-subtitle" class="block text-xs font-medium text-text-secondary mb-1.5">Subtitle</label>
					<input id="session-subtitle" type="text" bind:value={newSubtitle} placeholder="e.g. Mixed Levels"
						class="w-full px-4 py-3 bg-bg rounded-2xl border border-border/50 text-sm text-text-primary placeholder:text-text-tertiary focus:outline-none focus:ring-2 focus:ring-navy/20 transition-all" />
				</div>
				<div class="grid grid-cols-2 gap-3">
					<div>
						<label for="court-count" class="block text-xs font-medium text-text-secondary mb-1.5">Courts (× Rp 77K)</label>
						<input id="court-count" type="number" min="1" max="10" bind:value={newCourts}
							class="w-full px-4 py-3 bg-bg rounded-2xl border border-border/50 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-navy/20 transition-all" />
					</div>
					<div>
						<label for="racket-count" class="block text-xs font-medium text-text-secondary mb-1.5">Rackets (× Rp 20K)</label>
						<input id="racket-count" type="number" min="0" max="20" bind:value={newRackets}
							class="w-full px-4 py-3 bg-bg rounded-2xl border border-border/50 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-navy/20 transition-all" />
					</div>
				</div>
				...
				{/if}
				<button type="submit" class="w-full py-3.5 bg-navy text-white font-semibold text-sm rounded-2xl shadow-sm shadow-navy/20 hover:shadow-md transition-all active:scale-[0.98]">
					Create Session
				</button>
			</form>
		</div>
	...
	{#if showMapsConfig}
		<div class="bg-surface rounded-3xl border border-border/50 shadow-sm p-5 mb-5 animate-scale-in">
			...
			<textarea bind:value={mapsUrlInput} rows="3" placeholder="Paste embed URL / iframe tag / koordinat lat,lng"
				class="w-full px-4 py-3 bg-bg rounded-2xl border border-border/50 text-xs text-text-primary placeholder:text-text-tertiary focus:outline-none focus:ring-2 focus:ring-navy/20 transition-all resize-none"></textarea>
			<div class="grid grid-cols-2 gap-3 mt-3">
				<button disabled={isSavingMaps} onclick={() => (showMapsConfig = false)} class="py-2.5 bg-bg rounded-2xl text-sm font-medium text-text-secondary hover:bg-border/30 transition-all disabled:opacity-50">Cancel</button>
				<button disabled={isSavingMaps} onclick={saveMapsUrl} class="py-2.5 bg-navy text-white rounded-2xl text-sm font-semibold shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-50">
					{#if isSavingMaps}
	...
	<div class="bg-surface rounded-3xl border border-border/50 p-6 shadow-sm mt-5 overflow-hidden relative">
		...
		<div class="flex items-center gap-3 mb-6">
			<div class="w-10 h-10 rounded-xl bg-navy/5 flex items-center justify-center text-navy"><QrCode size={20} /></div>
			<div>
		...
		<div class="space-y-4">
			<div class="flex items-center gap-4 p-4 bg-bg rounded-2xl border border-border/50 group">
				<div class="w-20 h-20 bg-white rounded-xl border border-border/50 flex items-center justify-center overflow-hidden shadow-inner">
					{#if qrisPreview}
						<img src={qrisPreview} alt="Preview" class="w-full h-full object-contain" />
					{:else if db.settings?.qris_url}
						...
						<img
							...
							onerror={(e) => { e.currentTarget.src = 'https://placehold.co/80x80/f3f4f6/a1a1aa?text=QRIS'; }}
						/>
					{:else}
				...
				<div class="flex-1">
					...
					<p class="text-[10px] text-text-secondary leading-relaxed uppercase tracking-tight font-medium">Tampil di semua halaman checkout peserta.</p>
				</div>
			</div>
			<label class="relative flex items-center justify-center gap-2 py-4 bg-navy text-white text-xs font-black rounded-2xl cursor-pointer hover:bg-navy-dark transition-all active:scale-[0.98] shadow-lg shadow-navy/20 overflow-hidden group">
				...
				{#if qrisSuccess}
					<Check size={16} strokeWidth={3} class="animate-bounce" /><span>BERHASIL DIPERBARUI</span>
				{:else}
					<Upload size={16} strokeWidth={3} /><span>UPDATE QRIS IMAGE</span>
				{/if}
			</label>
		</div>
	</div>
	...
	<section class="mb-10 animate-fade-in-up" style="animation-delay:50ms">
		<div class="flex items-center justify-between mb-4 mt-6">
			...
			<input type="file" multiple accept="image/*" bind:this={galleryFileRef} onchange={handleUpload} class="hidden" />
		</div>

		<!-- Info: folder otomatis -->
		<div class="mb-4 px-4 py-3 bg-navy/5 rounded-2xl border border-navy/10 flex items-start gap-2">
			...
			<p class="text-[11px] text-text-secondary leading-relaxed">
				...
				<span class="font-bold text-navy font-mono">
					gallery/{new Date().getDate().toString().padStart(2,'0')}-{new Date().toLocaleString('en-US',{month:'long'})}-{new Date().getFullYear()}/
				</span>
				secara otomatis.
			</p>
		</div>
		...
		{#if db.gallery.length === 0 && localPreviews.length === 0}
			<div class="bg-surface rounded-3xl border border-border/50 p-8 text-center shadow-sm">
				...
				<p class="text-sm text-text-secondary font-bold">Belum ada foto gallery</p>
				<p class="text-[10px] text-text-tertiary mt-1 uppercase tracking-widest">Upload foto komunitas</p>
			</div>
		{:else}
			<div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
				...
				{#each db.gallery as photo (photo.id)}
					<div class="relative group aspect-square rounded-2xl overflow-hidden bg-bg border border-border/50 shadow-sm animate-scale-in">
						...
						<!-- Folder label -->
						{#if photo.folder}
							<div class="absolute top-1.5 left-1.5 px-1.5 py-0.5 bg-black/50 backdrop-blur-sm rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
								<span class="text-[8px] text-white font-bold truncate max-w-[60px] block">{photo.folder}</span>
							</div>
						{/if}
						<div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-end p-2">
							<button
				...
				{#each localPreviews as preview}
					<div class="relative aspect-square rounded-2xl overflow-hidden bg-bg border border-border/50 shadow-sm">
						<img src={preview} alt="Uploading..." class="w-full h-full object-cover opacity-50 grayscale" />
						<div class="absolute inset-0 flex flex-col items-center justify-center gap-1">
							<div class="w-5 h-5 border-2 border-navy border-t-white rounded-full animate-spin"></div>
							<span class="text-[8px] text-navy font-bold">Uploading</span>
						</div>
					</div>
				{/each}
							...
							<div class="flex items-center gap-3">
								<div class="flex-1 min-w-0">
									<div class="flex items-center gap-2 mb-1">
										...
										{#if isSessionPassed(session)}
											<span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-text-tertiary/10 text-text-tertiary text-[9px] font-bold">✓ ENDED</span>
										{:else if session.is_locked}
											<span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-warning/10 text-warning text-[9px] font-bold"><Lock size={8} />LOCKED</span>
										{/if}
									</div>
									<div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] sm:text-xs text-text-tertiary mt-1">
										<span class="flex items-center gap-1"><Calendar size={11} />{formatDate(session.date)} · {session.time}</span>
										<span class="flex items-center gap-1"><Users size={11} />{sessionParticipants.length} ({renters} rent)</span>
									</div>
									<div class="flex flex-wrap items-center gap-x-2 mt-1 text-[9px] sm:text-[10px] text-text-tertiary">
								...
								<div class="flex items-center gap-1.5 sm:gap-2">
									...
									<button
										...
										class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center transition-all active:scale-90 {session.is_locked ? 'bg-warning/10 text-warning' : 'bg-success/10 text-success'}"
									>{#if session.is_locked}<Unlock size={15} />{:else}<Lock size={15} />{/if}</button>
									<button
									...
									>{#if expanded}<ChevronUp size={15} />{:else}<ChevronDown size={15} />{/if}</button>
								</div>
							</div>
						</div>
						...
						{#if expanded}
							<div class="border-t border-border/50">
								{#if sessionParticipants.length === 0}
									<div class="p-4 text-center"><p class="text-xs text-text-tertiary">Belum ada peserta</p></div>
								{:else}
									<div class="divide-y divide-border/50">
										{#each getFilteredParticipants(session.id) as participant (participant.id)}
											{@const cost = calcPlayerCost(session, getParticipants(session.id), participant.needs_racket)}
											<div class="flex items-center gap-3 px-4 py-3 hover:bg-bg/40 transition-colors">
												<div 
												...
												>
													<div class="w-8 h-8 rounded-full {participant?.needs_racket ? 'bg-navy/10' : 'bg-navy/5'} flex items-center justify-center flex-shrink-0">
														<span class="text-xs font-bold text-navy">{participant?.name?.charAt(0)?.toUpperCase() || '?'}</span>
													</div>
													<div class="flex-1 min-w-0 pr-2">
														<p class="text-[13px] font-bold text-text-primary truncate">{participant?.name || 'Unknown'}</p>
														<p class="text-[9px] text-text-tertiary font-black tracking-wider uppercase">ID: {participant?.ticket_id || '---'}</p>
													</div>
												</div>
													<div class="flex items-center gap-2 mt-0.5">
														<span class="text-[9px] sm:text-[10px] {participant?.needs_racket ? 'text-navy/60' : 'text-text-tertiary'}">{participant?.needs_racket ? '🏸 Rent' : '🏸 Own'}</span>
														<span class="text-[9px] sm:text-[10px] text-text-tertiary font-medium">{formatCurrency(cost)}</span>
													</div>
												<div class="flex flex-col items-end gap-1">
													<button
														onclick={async () => await togglePaid(participant.id)}
														class="flex items-center gap-1 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[9px] sm:text-[11px] font-semibold transition-all active:scale-95 {participant.has_paid ? 'bg-success/10 text-success' : (participant.payment_status === 'rejected' ? 'bg-danger/10 text-danger border border-danger/20' : 'bg-text-tertiary/10 text-text-secondary')}"
													>
														{#if participant.has_paid}<Check size={12} />Lunas{:else if participant.payment_status === 'rejected'}<X size={10} />Ditolak{:else}<ClockIcon size={10} />Verify{/if}
													</button>
												</div>
												<button
	...
	<div class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm animate-fade-in flex flex-col justify-end sm:justify-center p-4">
		<div class="bg-surface w-full max-w-md mx-auto rounded-3xl shadow-2xl overflow-hidden animate-slide-up sm:animate-scale-in" onclick={(e) => e.stopPropagation()}>
			<div class="p-4 border-b border-border/50 flex justify-between items-center bg-bg/50">
				<h3 class="font-bold text-text-primary">Payment Verification</h3>
			...
			<div class="p-5 overflow-y-auto max-h-[70vh]">
				<div class="aspect-[3/4] bg-bg rounded-2xl border border-border/50 overflow-hidden mb-5">
					...
					<img
						...
						onerror={(e) => { 
							...
							e.currentTarget.src = 'https://placehold.co/600x800/f3f4f6/a1a1aa?text=Proof+Not+Found'; 
						}}
					/>
				</div>
				<div class="bg-bg rounded-2xl p-4 border border-border/50 space-y-3">
					...
					<div class="pt-3 border-t border-border/50 flex justify-between flex-wrap gap-2">
						<span class="px-2.5 py-1 rounded-md bg-white border border-border/50 text-[10px] font-bold text-text-secondary">
							{selectedVerification.needs_racket ? '🏸 Sewa Raket' : '🏸 Raket Sendiri'}
						</span>
						<span class="px-2.5 py-1 rounded-md bg-white border border-border/50 text-[10px] font-bold text-text-secondary">
							{selectedVerification.has_paid ? 'LUNAS' : 'PENDING'}
						</span>
					</div>
				</div>
			</div>
			<div class="p-4 bg-bg/50 border-t border-border/50 grid grid-cols-2 gap-3">
				{#if selectedVerification.has_paid}
					<button
						onclick={async () => { await togglePaid(selectedVerification.id); closeVerification(); }}
