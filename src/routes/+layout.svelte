...
<div class="min-h-dvh flex flex-col bg-bg">
	...
	{#if !isHomePage}
		<nav
			class="fixed bottom-0 inset-x-0 z-50 bg-surface/80 backdrop-blur-xl border-t border-border safe-area-bottom"
		>
			<div
				class="max-w-4xl mx-auto flex items-center justify-around h-16"
			>
				<a
					href="/"
					class="flex flex-col items-center gap-1 px-6 py-2 transition-colors duration-200 {currentPath ===
					'/'
						? 'text-navy'
				...
				>
					<Shield
						size={22}
						strokeWidth={currentPath === "/admin" ? 2.5 : 1.5}
					/>
					<span class="text-[10px] font-semibold tracking-wide"
						>Admin</span
					>
